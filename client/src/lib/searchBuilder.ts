...
export function buildGoogleSearchUrl(data: SearchData): string {
  const { jobTitle, selectedSites, locations, remoteWork, timeline, region, exactMatch } = data;

  // Build the main search query - wrap job title in quotes if exact match is enabled
  const processedJobTitle = exactMatch ? `"${jobTitle}"` : jobTitle;
  let searchTerms = [processedJobTitle];
